...
class HaierSelect(SelectEntity):
    ...
    def current_option(self) -> str | None:
        ...
        formatter = getattr(self._device, "get_select_option", None)
        if formatter is not None:
            value = formatter(self._device_attr_name, value)
        if value in (None, "None", "unknown"):
            return None
        value = str(value)
        if value not in self._attr_options:
            # Home Assistant requires current_option to be one of options.
            # Program titles can arrive from WebSocket after entity creation,
            # so add the current value dynamically instead of showing unknown.
            self._attr_options = [value, *self._attr_options]
        return value
